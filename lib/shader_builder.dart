...
class CustomAnimatedSampler extends StatelessWidget {
  /// Create a new [CustomAnimatedSampler].
  const CustomAnimatedSampler(
    this.builder, {
    required this.child,
    super.key,
    this.enabled = true,
...
class _ShaderSamplerBuilderLayer extends OffsetLayer {
  ...
  void addToScene(ui.SceneBuilder builder) {
    ...
    try {
      ...
      // Suppose we want to clip the image to a particular rect
      ...
      // callback(image, Size(size.width, 100), canvas);
      callback(image, size, canvas);
    } finally {
      image.dispose();
    }
...
class MinuteChangeShaderEffect extends StatefulWidget {
  const MinuteChangeShaderEffect({Key? key, required this.child})
      : super(key: key);
...
class _MinuteChangeShaderEffectState extends State<MinuteChangeShaderEffect>
  ...
  void _checkMinuteChange() {
    _timer = Timer.periodic(Duration(milliseconds: 1), (Timer t) async {
      int currentMinute = DateTime.now().minute;
      int currentSec = DateTime.now().second;

      if (currentMinute != _prevMinute) {
        // final diff = currentSec - _prevSec;
        // if (currentMinute == _prevMinute && diff == 30) {
        _prevMinute = currentMinute;
        _prevSec = currentSec;
        _controller.reset();

        // Haptic feedback to indicate the impact is about to start
        HapticFeedback.heavyImpact();

        if (Platform.isAndroid) {
          await fakeAwait(milliseconds: 1000);
        }
        _controller.forward();
        HapticFeedback.vibrate();
      }
      print(currentSec - _prevSec);
      secondsUntilImpact = currentSec - _prevSec;
      if (currentSec - _prevSec == 55) {
  ...
  void dispose() {
    _controller.dispose();
    _timer.cancel(); // Don't forget to dispose of the timer!
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    if (secondsUntilImpact != null &&
        secondsUntilImpact! <= 55 &&
        secondsUntilImpact! >= 10) {
      print("secondsUntilImpact ${secondsUntilImpact}");
      return widget.child;
    }
    return ShaderBuilder(
      (context, shader, child) {
        return CustomAnimatedSampler(
          (image, size, canvas) {
  ...
  Future<void> fakeAwait({required int milliseconds}) async {
